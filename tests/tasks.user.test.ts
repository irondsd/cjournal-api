...
describe('tasks by user', () => {
    ...
    test('get all activities', async () => {
        const activities = await Request(token, `users/${_id}/tasks`, 'GET')
    ...
    test('create tasks by user', async () => {
        ...
        const task = await Request(token, `users/${_id}/tasks/`, 'POST', test_task)
        expect(task).toHaveProperty('_id', task_id)
    })

    test('get created tasks', async () => {
        const task = await Request(token, `users/${_id}/tasks/${task_id}`, 'GET')
        expect(task).toHaveProperty('_id', task_id)
    ...
    test('edit created tasks', async () => {
        ...
        test_task.activity_type = 'Alcohol'
        const task = await Request(token, `users/${_id}/tasks/${task_id}`, 'PUT', test_task)
        expect(task).toHaveProperty('_id', task_id)
        expect(task).toHaveProperty('comment', test_task.comment)
    })
    ...
    test('delete created tasks', async () => {
        test_task.comment = 'edited test tasks'
        test_task.activity_type = 'Alcohol'
        const task = await Request(token, `users/${_id}/tasks/${task_id}`, 'DELETE')
